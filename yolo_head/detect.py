...
def detect(opt,save_img=False):
    ...
    save_txt = True
    webcam = False
    tracker = sv.ByteTrack(track_activation_threshold=0.5,lost_track_buffer=30,minimum_matching_threshold=0.70,minimum_consecutive_frames=1)
        ...
        for i, det in enumerate(combined_pred):  # detections per image
            ...
            if len(det):
                ...
                for frame_detection in detection_with_tracks:
                    ...
                    accumulate_head_track(bbox,confs,clss,person_id,names_V_K,names_K_V,accumulator,im0,p,colors)
...
def accumulate_head_track(bbox,conf,cls,person_id,names_V_K,names_K_V,accumulator,im0,p,colors):
    if cls == names_V_K.get('head', None): #only track the head 
        ...
        cv2.putText(im0, str(person_id), (int(bbox[0]), int(bbox[3]) + 15), cv2.FONT_HERSHEY_SIMPLEX, 0.9, colors[int(cls)], 2)
