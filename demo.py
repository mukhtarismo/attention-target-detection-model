...
parser.add_argument('--model_weights', type=str, help='model weights', default='pretrained-models/model_demo.pt')
parser.add_argument('--image_dir', type=str, help='images', default='data/demo/Frames2')
parser.add_argument('--head', type=str, help='head bounding boxes', default='data/demo/heads.txt')
...
def run():
    ...
    with torch.no_grad():
        ...
        for i in df.index:
            ...
            frame_raw = cv2.rectangle(frame_raw, (int(head_box[0]), int(head_box[1])), (int(head_box[2]), int(head_box[3])), (255, 0, 0) , 2) 
            if args.vis_mode == 'arrow':
                if inout < args.out_threshold: # in-frame gaze
                    ...
                    # ax.add_patch(circ)
                    # print(head_box)
                    print(int((head_box[1]+head_box[3])/2))
                    print(int((head_box[0]+head_box[2])/2))
                    frame_raw = cv2.circle(frame_raw, (int(norm_p[0]*width), int(norm_p[1]*height)), int(height/50.0), (255, 0, 0), 2) 
                    cv2.arrowedLine(frame_raw,(int((head_box[0]+head_box[2])/2),int((head_box[1]+head_box[3])/2)),(int(norm_p[0]*width),int(norm_p[1]*height)), (230,253,11),thickness=3)
                    # int(head_bboxes[1]+head_bboxes[3])//2,int(head_bboxes[0]+head_bboxes[2])//2
            ...
            # dir_path  = '/output-frames/{0}.jpg'.format(file_count_1)
            os.path.join(base_path,'output-frames', i)
            cv2.imwrite(os.path.join(base_path,'output-frames', i), frame_raw) 
